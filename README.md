...
| Y036 | Y036 detects common errors in `__exit__` and `__aexit__` methods. For example, the first argument in an `__exit__` method should always be annotated with `object` or `type[BaseException] \| None`.
