...
class PyiVisitor(ast.NodeVisitor):
    ...
    def _check_union_members(
        self, union_node: ast.AST, members: Sequence[ast.AST]
    ) -> None:
        if len({ast.unparse(member) for member in members}) < len(members):
            self.error(union_node, Y016)
    ...
    def visit_BinOp(self, node: ast.BinOp) -> None:
        ...
        self._check_union_members(node, members)
    ...
    def visit_Subscript(self, node: ast.Subscript) -> None:
        ...
        if (
            ...
            self._check_union_members(node, node.slice.elts)
